...
public class HuffmanCode {
    ...
    //输出huffmanCode
    public static void outputHuffmanCode(NodeCode[] huffmanCode, int n) {
    ...
    public static void efficient(Node[] huffmanTree, int m, NodeCode[] huffmanCode, int n) {
        int sum = 0, j = 0, k;
        for (int i = 0; i < m; i++) {
            if (j < n) {
                k = huffmanCode[j].code.length();
                sum = sum + k * huffmanTree[i].weight;
                j++;
            }
        }
        ...
        double H = 0;
        for (int i = 0; i < m; i++) {
            H = H - huffmanTree[i].weight * (Math.log((double) huffmanTree[i].weight) / Math.log((double) 2));
        }
    ...
    public static int log(double value, int base) {
    ...
    public static void main(String[] args) {
        ...
        //输出huffmanCode编码
        outputHuffmanCode(huffmanCode, n);

        efficient(huffmanTree, n, huffmanCode, m);
    }
}
